...
const Checkout = ({ history }) => {
  ...
  const applyDiscountCoupon = () => {
    ...
    applyCoupon(user.token, coupon).then((res) => {
      console.log("RES ON COUPON APPLYED", res.data);
      if (res.data) {
        setTotalAfterDiscount(res.data);
        // update redux coupon applyed true/false
        dispatch({
          type: "COUPON_APPLIED",
          payload: true,
        });
  ...
  const createCashOrder = () => {
    createCashOrderForUser(user.token, COD, couponTrueOrFalse).then((res) => {
      ...
      // empty cart for redux, localStorage, reset coupon, reset COD, redirect
      if (res.data.ok) {
        // empty local storage
